...
(eval-when (:compile-toplevel :load-toplevel :execute)
  (defpackage #:org.foldr.michaelw.diff-sexp
    (:use "CL")
    (:export #:diff-sexp)))

;(in-package #:org.foldr.michaelw.diff-sexp)
...
(defun min/edit (record &rest records)
  ...
  (flet ((min/edit/2 (a b)
	   (if (<= (edit-distance a) (edit-distance b)) a b)))
...
(defun initial-distance (function list)
  ...
  (loop with seq = (make-sequence '(vector edit-record) (1+ (length list)) 
	...
	for i from 0
	for elt in list do
	(setf (elt seq (1+ i)) (extend-compound-record (elt seq i) (funcall function elt)))
	finally (return seq)))
